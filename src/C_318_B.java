...
public class C_318_B {
    ...
    public static void main(String[] args) {
        ...
        for(int i=0;i<n;i++){
            for(int j=i+1;j<n;j++){
                if(Collections.binarySearch(v[i].l, j)>=0 && v[i].l.get(Collections.binarySearch(v[i].l, j))==j)
                for(int k=j+1;k<n;k++){
                    if(Collections.binarySearch(v[j].l, k)>=0 && v[j].l.get(Collections.binarySearch(v[j].l, k))==k && Collections.binarySearch(v[i].l, k)>=0 && v[i].l.get(Collections.binarySearch(v[i].l, k))==k){
